...
@Query(value="select * from book where name like %?1% or description like %?1% or page_count like %?1% or author like %?1%",nativeQuery=true)
public List<Book> findAllSearchAllFields(String search);
@Query(value="select * from book where name like %?1% or description like %?1% or page_count like %?1% or author like %?1% limit ?2,?3",nativeQuery=true)
